...
def main():
    ...
    try:
        ...
        ipaddresslist = zk.get(CLUSTER_NODES)[0].decode().split()
        zk.set(CLUSTER_NODES, '{},{}'.format(ipaddresslist, get_ip_address()).encode())
        if(zk.exists):
            start_node(ipaddresslist)
        else:
            bootstrap_cluster()
        wait_until_primary()
        """if (zk.exists(CLUSTER_NODES) == None)
            bootstrap_cluster()
            wait_until_primary()
        zk.set(CLUSTER_NODES, ipaddr.encode())
